...
class FisherFaces:
    ...
    def _lda(self):
        ...
        W_A = self.eigenvecs_pca
        Sb_PCA = W_A.T @ Sb @ W_A
        Sw_PCA = W_A.T @ Sw @ W_A
        eigenvals, eigenvecs = np.linalg.eig(np.linalg.inv(Sw_PCA)*Sb_PCA)
    ...
    def _pca(self):
        ...
        U, sigma, _ = np.linalg.svd(mean_sub, full_matrices=False)
        U = U[:,0:self.num_components]
        self.eigenvecs_pca = U
    ...
    def read_project(self,image_path):
    ...
    def train(self):
        ''' Run training '''
        self._lda()
        self.fisher_eigenvectors = np.dot(self.eigenvecs_lda.T, self.eigenvecs_pca.T) #Final W in the formula